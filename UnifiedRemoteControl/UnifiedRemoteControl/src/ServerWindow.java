...
public class ServerWindow implements ActionListener {
	...
	private JFrame window = new JFrame("Unified Remote Control");
	...
	public class RemoteDataServer implements Runnable {
		...
		public void run() {
			...
			try {
				InetAddress ip = InetAddress.getLocalHost();
				serverMessages.setText("Waiting for connection on "
				...
				serverMessages.setText("Port " + PORT
			...
			while (connected) {
				...
				try {
					...
					if (message.equals("Connectivity")) {
						...
						serverMessages
								.setText("Controller has Disconnected. Trying to reconnect."); // echo
																								// the
																								// message
																								// back
					} else {
